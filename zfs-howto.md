...
Contents:
 ...
 - [Tutorial 3: Working with ZFS send streams as local files](#user-content-tutorial-3-working-with-zfs-send-streams-as-local-files)

   - [Step 0 - Save a full send stream to a file](#user-content-step-0---save-a-full-send-stream-to-a-file)
   - [Step 1 - Generate a resume token from a partial stream](#user-content-step-1---generate-a-resume-token-from-a-partial-stream)
...
ZFS send streams can be saved as files and manipulated offline using the
.Sy zstream
utility. This is useful for:

 - Resuming an interrupted receive without contacting the origin (source) host
 - Chunking a large stream for transport over size-limited channels
 - Splitting a stream into pieces, transferring them separately, and reassembling
 - Generating resume tokens from a partial stream file on a slave host

All commands in this tutorial assume the patched kernel/userspace tools that
include the
.Sy stream_offset
field in resume tokens, enabling O(1) resume via byte offset seek instead of
an O(n) linear scan.

### Setup

In order to demonstrate how new features work we have to setup following schenario:
- local test-host system with data zpool;
- remote bkp-host system with data-bkp zpool;
...
### Step 0 - Save a full send stream to a file
	...
	# Put some data on fs
	dd if=/dev/urandom of=/data/test.bin bs=1M count=1

	# Create a snapshot:
	zfs snapshot  data@20260523

	# For a non-recursive send of a single filesystem:
	zfs send data@20260523 > /tmp/data-fs.zfs


-------------
### Step 1 - Generate a resume token from a partial stream

When a stream file is truncated (interrupted download, partial transfer),
you can generate a resume token directly from the file — no need to query
the origin host.

	# Generate truncated stream
	head -c 8K /tmp/data-fs.zfs > /tmp/truncated-stream.zfs

	# Generate a resume token from the partial stream:
	zstream token -g -i /tmp/truncated-stream.zfs > /tmp/resume-token

        # Inspect the token to find the byte offset:
        zstream token $(cat /tmp/resume-token)

        # The output includes "stream_offset: 0x..." — that's the byte position
        # where the stream ends.

-------------
### Step 2 - Resume a send from a partial stream file

Resume sending from a saved stream file, starting at the point recorded in
the resume token. The patched kernel stores the byte offset in the token,
so
.Sy zstream resume
can seek directly to the resume point (O(1)) instead of scanning linearly.


	# Send truncated stream to backup host (expect failure)
	cat /tmp/truncated-stream.zfs | ssh bkp-host zfs recv -s -F data-bkp

	# Should be non-zero (recv fails on truncated stream)
	echo "zfs recv exit code: $?"

	# Send the rest of stream from stream_offset in resume token
	zstream resume -t $(cat /tmp/resume-token) -i /tmp/data-fs.zfs | ssh bkp-host zfs recv -s -F data-bkp

	# Should be zero (recv completes successfully)
	echo "zfs recv exit code: $?"
...
Break a full send stream into fixed-size, record-aligned chunks. Each chunk
is a self-describing stream (starts with DRR_BEGIN), suitable for transfer
over size-limited channels or storage.

	# Split into ~128K chunks:
	zstream split -c 128K -i /tmp/data-fs.zfs -o /tmp/data-fs-chunk

	# This creates:
	#   /tmp/data-fs-chunk.0000  (first chunk, contains DRR_BEGIN)
	#   /tmp/data-fs-chunk.0001
	#   /tmp/data-fs-chunk.0002
	#   ...

	# Split from stdin (pipe-compatible, no seek needed):
	zfs send data@20260523 | zstream split -c 128K -o /tmp/data-fs-chunk


-------------
### Step 4 - Join chunks back into a single stream

Reassemble the split chunks into a complete send stream.

	# Join all chunks (head is the first chunk, rest are partials):
	zstream join -i /tmp/data-fs-chunk.* > /tmp/data-fs-reassembled.zfs

	# Exit codes:
	#   0 — stream is complete (DRR_END was written)
	#   1 — error
	#   2 — stream is incomplete (no DRR_END; expected when using -c on resume)

	# Receive the reassembled stream:
	zstream join -i /tmp/data-fs-chunk.* | ssh bkp-host zfs receive -F data-bkp


-------------
### Step 5 - upload zfs snaphots to remote file server with resume 

A realistic scenario: upload a stream in chunks over an unreliable link,
resume from where you left off, then join and receive.
...
A master generates one snapshot stream. Slaves save it locally and can
resume independently without contacting the master.

	# === On the any remote host ===

	ssh bkp-host cat /tmp/full-stream.zfs | head -c 200K | zfs recv -s -F data
	
	while [ $? -ne 0 ]
	do
		token=$(zfs get resume_token data)
		ssh bkp-host zstream resume -t ${token} -i /tmp/full-stream.zfs | zfs recv -s -F data
	done
